...
function registrasi($data){
    ...
    // masukan data kedalam elemen

    $username = strtolower(stripslashes($data["username"])); //memaksa data yg di input menjadi kecil dan tidak dapat menggunakan slash
    $password = mysqli_real_escape_string($conn, $data["password"]); //password dapat menggunakan kutip dalam password
    $konfirmasipassword = mysqli_real_escape_string($conn, $data["konfirmasipassword"]);


    //cek apakah nama user sama 
    $result = mysqli_query($conn, "SELECT username FROM users WHERE username = '$username'");

    if( mysqli_fetch_assoc($result)){
        echo "
        <script>
            alert('username yang di masukan sudah ada');
        </script>
    ";
    return false; 
    }

    //cek apakah password yang di input sama
    if($password !== $konfirmasipassword){
        echo "
        <script>
            alert('password yang di masukan tidak sama');
        </script>
    ";
    return false;
    }
